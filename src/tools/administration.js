...
export const administrationTools = Object.freeze([
  ...
  D('create_site', 'Create a site through the preview customer-site API.', { customerId: id('Customer identifier.'), body: siteBody }, ['customerId', 'body'], ['POST /api/customers/{customerId}/sites'], 'write', (a) => organizations.createSite(a.customerId, a.body)),
  ...
  D('list_user_roles', 'List preview user roles for one organization unit.', { orgUnitId, ...page }, ['orgUnitId'], ['GET /api/org-units/{orgUnitId}/user-roles'], 'read', (a) => users.listUserRoles(a.orgUnitId, a)),
  D('get_user_role', 'Retrieve one preview user role by organization and role IDs.', { orgUnitId, userRoleId: id('User-role identifier.') }, ['orgUnitId', 'userRoleId'], ['GET /api/org-units/{orgUnitId}/user-roles/{userRoleId}'], 'read', (a) => users.getUserRole(a.orgUnitId, a.userRoleId)),
  ...
  D('create_user_role', 'Create a preview user role beneath one organization unit.', { orgUnitId, body: userRoleBody }, ['orgUnitId', 'body'], ['POST /api/org-units/{orgUnitId}/user-roles'], 'write', (a) => users.createUserRole(a.orgUnitId, a.body)),
