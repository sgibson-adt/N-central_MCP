...
export const reportingTools = Object.freeze([
  D('report_devices_bulk', 'Retrieve one selected data class across an organization device inventory with concurrency capped at five.', { orgUnitId, dataType: { type: 'string', enum: ['assets', 'monitoring', 'customProperties'] }, concurrency: { type: 'integer', minimum: 1, maximum: 5 } }, ['orgUnitId', 'dataType'], ['GET /api/org-units/{orgUnitId}/devices', 'GET /api/devices/{deviceId}/assets', 'GET /api/devices/{deviceId}/service-monitor-status', 'GET /api/devices/{deviceId}/custom-properties'], reports.reportDevicesForOrg),
  D('report_all_users_by_service_org', 'Build a bounded deduplicated user view beneath one service organization.', { soId: id('Service organization identifier.') }, ['soId'], ['GET /api/service-orgs/{soId}/customers', 'GET /api/org-units/{orgUnitId}/users'], reports.reportAllUsersByServiceOrg),
  ...
  D('report_customer_site_summary', 'Summarize customers and their sites with bounded fan-out.', { customerId: id('Optional customer identifier.') }, [], ['GET /api/customers', 'GET /api/customers/{customerId}/sites'], reports.reportCustomerSiteSummary),
